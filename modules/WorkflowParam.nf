...
process WorkflowParam { // create a file with the workflow parameters in out_path
    label 'bash'
    publishDir "${params.out_path}/reports", mode: 'copy', overwrite: false
    cache 'false'
    ...
    echo "execution mode": ${params.system_exec} >> Run_info.txt
    ...
    echo "result path: ${params.out_path}" >> Run_info.txt
    ...
    echo -e "\\n\\nUSER VARIABLES:\\n\\nout_path: ${params.out_path}" >> Run_info.txt
    """
}
//${projectDir} nextflow variable
//${workflow.commandLine} nextflow variable
//${workflow.manifest.version} nextflow variable
//Note that variables like ${params.out_path} are interpreted in the script block
