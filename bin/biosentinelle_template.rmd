...
read_tsv_with_dummy <- function(file_path, row_nb){
    if( ! file.exists(file_path)){
        ...
        if (nrow(df) == 0){
            ...
            df <- df[ -(1:nrow(df)), ]
        }else if(nrow(df) > row_nb){
            df <- df[1:row_nb, ]
            truncated <- TRUE
        }
        return(list(df = df, truncated = truncated, nb = row_nb))
    }
}
```
...
```{r, echo=FALSE, results='asis'}
    fastq_name <- params$fastq_name
    nb_ref <- params$nb_ref
...
Number of reference genomes tested : ```r nb_ref```
...
<span style="color:green">snp_counts.tsv file in the [tsv](./tsv) folder</span>
        '))
        tempo_read <- read_tsv_with_dummy("tsv/snp_counts.tsv", row_nb = 10)
        df <- tempo_read$df
...
Number of features tested : ```r nb_feat```
...
<span style="color:green">coverage.tsv file in the [tsv](./tsv) folder</span>
        '))
        tempo_read <- read_tsv_with_dummy("tsv/coverage.tsv", row_nb = 10)
        df <- tempo_read$df
        if(nrow(df) > 1){
...
The insertion dashboard summarizes junction reads found between feature alignments and reference genome alignments:

<p>
  <a href="./dashboard/index.html" target="_blank" rel="noopener">Open the insertion dashboard</a>
</p>

<iframe src="./dashboard/index.html" title="BioSentinel insertion dashboard" style="width:100%; height:760px; border:1px solid #dde5ef; border-radius:8px;"></iframe>
